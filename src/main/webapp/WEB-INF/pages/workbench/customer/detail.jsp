...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#div_customerRemarks").on("mouseover",".remarkDiv",function (){
			$(this).children("div").children("div").show();
		})
		$("#div_customerRemarks").on("mouseout",".remarkDiv",function (){
			$(this).children("div").children("div").hide();
		})
		$("#div_customerRemarks").on("mouseover",".myHref",function (){
			$(this).children("span").css("color","red");
		})
		$("#div_customerRemarks").on("mouseout",".myHref",function (){
			$(this).children("span").css("color","#E6E6E6");
		})
		...
		$("#saveCreateCustomerRemarkBtn").click(function (){
			...
			$.ajax({
				url:'workbench/customer/createCustomerRemark.do',
				data:{
				...
				success:function (data){
					if (data.code=="1"){
						...
						Str+="			<font color=\"gray\">客户</font> <font color=\"gray\">-</font> <b>${customer.name}</b> <small style=\"color: gray;\"> "+data.retData.createTime+" 由${sessionScope.sessionUser.name}创建</small>";
		...
		//给客户备注删除标签绑定单击事件
		$("#div_customerRemarks").on("click","a[name='deleteA']",function (){
			//收集参数，获取该备注id
		...
		//给客户备注“修改”标签绑定单击事件
		$("#div_customerRemarks").on("click","a[name='editA']",function (){
			//给修改客户备注模态窗口初始化
			var id = $(this).attr("remarkId");
			var noteContent = $("#div_"+id+" h5").text();
			$("#edit-noteContent").val(noteContent);

			$("#Hidden_CustomerRemarkId").val(id);
		...
		$("#updateRemarkBtn").click(function (){
			//收集参数
			var id = $("#Hidden_CustomerRemarkId").val();
     ...
     //给交易的删除标签绑定单击事件
		$("#tBobyTran a[name='deleteA']").click(function (){
			//获取该交易的id
		...
		//给删除交易的模态窗口的”删除“按钮绑定单击事件
		$("#deleteTranBtn").click(function (){
			//收集参数
			var tranId=$("#tranId").val();
			//发送请求
			$.ajax({
				url:'workbench/customer/deleteTranFromCustomerDetail.do',
				data:{
					tranId:tranId
				},
				type:'post',
...
<body>
	...
	<!-- 备注 -->
	<div style="position: relative; top: 10px; left: 40px;" id="div_customerRemarks">
		<div class="page-header">
			<h4>备注</h4>
		</div>
		<c:forEach items="${customerRemarkList}" var="obj">
			<div class="remarkDiv" style="height: 60px;" id="div_${obj.id}">
				...
				<div style="position: relative; top: -40px; left: 40px;" >
					...
					<font color="gray">客户</font> <font color="gray">-</font> <b>${customer.name}</b> <small style="color: gray;"> ${obj.editFlag=="0"?obj.createTime:obj.editTime} 由${obj.editFlag=="0"?obj.createBy:obj.editBy}${obj.editFlag=="0"?'创建':'修改'}</small>
					<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">
						<a remarkId="${obj.id}" name="editA" class="myHref" href="javascript:void(0);"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #E6E6E6;"></span></a>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<a remarkId="${obj.id}" name="deleteA" class="myHref" href="javascript:void(0);"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>
					</div>
				</div>
			</div>
		</c:forEach>
	...
	<div class="modal fade" id="editRemarkModal" role="dialog">
		<input type="hidden" id="Hidden_CustomerRemarkId">
		<%-- 备注的id --%>
	...
	<div>
		<div style="position: relative; top: 20px; left: 40px;">
			...
			<div style="position: relative;top: 0px;">
				<table id="activityTable2" class="table table-hover" style="width: 900px;">
					...
					<c:forEach items="${tranList}" var="obj">
						<tr id="tr_${obj.id}">
							<td><a  name="tranDetailA" href="workbench/transaction/toTranDetail.do?tranId=${obj.id}" style="text-decoration: none;">${customer.name}-${obj.name}</a></td>
							<td>${obj.money}</td>
							<td>${obj.stage}</td>
							<td>${obj.possibility}</td>
							<td>${obj.expectedDate}</td>
							<td>${obj.type}</td>
							<td><a name="deleteA" tranId="${obj.id}" href="javascript:void(0);"  style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>删除</a></td>
						</tr>
					</c:forEach>
						<%--<tr>
			...
			<div>
				<a href="workbench/transaction/toSave.do?customerName=${customer.name}&customerId=${customer.id}" style="text-decoration: none;"><span class="glyphicon glyphicon-plus" id="creatTranA"></span>新建交易</a>
			</div>
		</div>
	</div>
		...
		<div style="position: relative; top: 20px; left: 40px;">
			...
			<div style="position: relative;top: 0px;">
				<table id="activityTable" class="table table-hover" style="width: 900px;">
					...
					<tbody>
					<c:forEach items="${contactsList}" var="obj">
						<tr>
							<td><a href="workbench/contacts/toContactsDetail.do?contactsId=${obj.id}" style="text-decoration: none;">${obj.fullname}</a></td>
							<td>${obj.email}</td>
							<td>${obj.mphone}</td>
							<td><a contactsId="obj.id" href="javascript:void(0);" data-toggle="modal" data-target="#removeContactsModal" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>删除</a></td>
						</tr>
					</c:forEach>
						<%--<tr>

...
<script type="text/javascript" src="jquery/bootstrap-datetimepicker-master/locale/bootstrap-datetimepicker.zh-CN.js"></script>
	...
	<script type="text/javascript">
		$(function (){
			...
			$("#tranId").val('${tran.id}');
			//设置所有者默认选中
			var selected1=$("#create-transactionOwner option")
			$.each(selected1,function (index,obj){
				if ($(this).html()=='${tran.owner}'){
					$(this).prop("selected",true);
				}
			})
			//设置交易阶段默认选中
			var selected2=$("#create-transactionStage option")
			$.each(selected2,function (index,obj){
				if ($(this).html()=='${tran.stage}'){
					$(this).prop("selected",true);
				}
			})

			//设置来源默认选中
			var selected3=$("#create-clueSource option")
			$.each(selected3,function (index,obj){
				if ($(this).html()=='${tran.source}'){
					$(this).prop("selected",true);
				}
			})

			//设置类型默认选中
			var selected4=$("#create-transactionType option")
			$.each(selected4,function (index,obj){
				if ($(this).html()=='${tran.type}'){
					$(this).prop("selected",true);
				}
			})
			...
			$("#create-accountName").val('${tran.customerId}');
			$("#create-activitySrc").attr("activityId",'${tran.activityName}');  //     activityName存的是id
			$("#create-activitySrc").val('${tran.activityId}')                      //activityId存的是name
			$("#create-contactsName").attr("contactsId",'${tran.contactsName}');    //contactsName存的是tran的id
			$("#create-contactsName").val('${tran.contactsId}');     //contactsId存的是tran的名称
			...
			$("#create-possibility").val('${tran.possibility}');
			...
			$("#saveBtn").click(function (){
				...
				var activityId=$("#create-activitySrc").attr("activityId");  //     activityName存的是id
				var contactsId=$("#create-contactsName").attr("contactsId");    //contactsName存的是tran的id
				...
				//发送请求
				$.ajax({
					url:'workbench/transaction/saveUpdateTranById.do',
					data:{
					id   	        : 	id   	       ,
					owner	        : 	owner	       ,
					money           : 	money          ,
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" class="form-control" style="width: 300px;" id="searchActivityByName" placeholder="请输入市场活动名称，支持模糊查询">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" class="form-control" style="width: 300px;" id="searchContactsText" placeholder="请输入联系人名称，支持模糊查询">
						    <span class="glyphicon glyphicon-search form-control-feedback"></span>
						  </div>
						</form>
					</div>
